...
<div class="container-fluid">
    ...
    <div class="col-md-12">
        <div class="card-body" style="margin-top: -10px">
            <form method="GET" action="{{ url('/ad-se-vie-br') }}"  enctype="multipart/form-data">
                <div class="row" >
                    <div class="col-sm-1" >
                        <h6 style="color: gray">Bihar :<h6>
                    </div>
                    <div class="col-sm-2" >
        ...
        <div class="card-body" style='margin-top:-55px'>
            <div class="row ">
                ...
                <div class="col-sm-6" style='text-align: right'>
                    <a href="{{route('excel.br',['application_no'=>request('application_no'),'mem_nm'=>request('mem_nm'), 'post_applied_for'=>request('post_applied_for')])}}"><button type='submit' class='button22'>Export To Excel</button></a>
                </div>
            </div>
        </div>
        <div class="card-body"style='margin-top:-40px'>
            ...
            <table   id="customers"  width=100%>
                ...
                @foreach ($brappli as $br)
                    <tr >
                        <td>{{ $br["application_no"]}}</td>
                        <td>{{ $br["mem_nm"]}}</td>
                        <td>{{ $br["guard_nm"]}}</td>
                        <td>{{ $br["mem_cast"]}} </td>
                        <td>{{ $br["birth_dt"]}}</td>
                        <td>{{ $br["post_applied_for"]}}</td>
                        <td>{{ $br["mem_age"]}}</td>
                        <td>{{ $br["gender"]}}</td>
                        <td>
                            @if($br["mail_count"] > 0)
                                <a href ="{{url('send-mail/'.$br['mem_id'])}}"class="button24 button1"  style="text-decoration:none;">Resend Mail </a>&nbsp;&nbsp;
                            @else
                                <a href ="{{url('send-mail/'.$br['mem_id'])}}"class="button24 button1"  style="text-decoration:none;">Send Mail </a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                            @endif
                                <a href ="{{url('pdf-br/'.$br['mem_id'])}}"class="button24"  style="text-decoration:none;" target="_blank">Print </a>
                        </td>
