            ...
            {{-- <tr>  --}}
                ...
                {{-- <td width="100%"> --}}
                    <div class="card-body" style="margin-top: -10px">
                        <form method="GET" action="{{ url('ad-se-vie-wb') }}"  enctype="multipart/form-data">
                            <div class="row" >
                                <h6 style="color: gray">WEST BENGAL :</h6>
                                <div class="col-sm-2" >
                                    {{-- <input type="text" value="{{request('application_no')}}" id="application_no" class="register_input" name="application_no" autocomplete="off" />
                                    <label for="register_input" placeholder="Enter Application No "></label> --}}
                                ...
                                <div class="col-sm-2" >
                                    {{-- <input type="text" value="{{request('mem_nm')}}" id="mem_nm" class="register_input" name="mem_nm" autocomplete="off" />
                       ...
                       {{-- <div class="col-sm-4" style='text-align: right'>
                            <a href="{{url('#')}}"><button type='button' class=''>Export To Excel</button></a>
                        </div>  --}}
                    </div>
                    <div class="card-body" >
                        <div class="row ">
                            <div class="col-sm-6" ></div>
                            <div class="col-sm-6" style='text-align: right;margin-top:-55px'>
                                <a href="{{route('excel.wb',['application_no'=>request('application_no'),'mem_nm'=>request('mem_nm'), 'post_applied_for'=>request('post_applied_for')])}}"><button type='button' class='button22 '>Export To Excel</button></a>
                            </div>
                        </div>
                    </div>
                    <div class="card-body"style='margin-top:-70px'>
                        ...
                        <table id="customers" width=100%>
                            ...
                            @foreach ($wbappli as $wb)
                                <tr >
                                    <td>{{ $wb["application_no"]}}</td>
                                    <td>{{ $wb["mem_nm"]}}</td>
                                    <td>{{ $wb["guard_nm"]}}</td>
                                    <td>{{ $wb["mem_cast"]}} </td>
                                    <td>{{ $wb["birth_dt"]}}</td>
                                    <td>{{ $wb["post_applied_for"]}}</td>
                                    <td>{{ $wb["mem_age"]}}</td>
                                    <td>{{ $wb["gender"]}}</td>
                                    <td>
                                        @if($wb["mail_count"] > 0)
                                            <a href ="{{url('send-mail-wb/'.$wb['mem_id'])}}"class="button24 button1" style="text-decoration:none;">Resend Mail </a>&nbsp;&nbsp;
                                        @else
                                            <a href ="{{url('send-mail-wb/'.$wb['mem_id'])}}"class="button24 button1" style="text-decoration:none;">Send Mail </a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                                        @endif
                                            <a href ="{{url('pdf-wb/'.$wb['mem_id'])}}"class="button24" style="text-decoration:none;" target="_blank">Print </a>
                                    </td>
                                </tr>
                            @endforeach
                         </table>
